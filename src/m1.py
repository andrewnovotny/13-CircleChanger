...
class CircleChanger(object):
    ...
    def get_distance_from(self, point):
        ...
        ################################################################
        dist = self.circle.center.get_distance_from(point)
        return dist
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        randcolor = self.colors[random.randrange(0, length)]
    ...
    def change_to_next_color_in_tuple(self):
        ...
        ################################################################
        length = len(self.colors)
        if self.circle.fill_color in self.colors:
            current = self.colors.index(self.circle.fill_color)
            self.circle.fill_color = self.colors[(current + 1) % (length)]
        else:
